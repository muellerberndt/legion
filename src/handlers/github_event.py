...
class GitHubEventHandler(Handler):
    ...
    async def handle(self) -> HandlerResult:
        ...
        try:
            ...
            self.logger.debug(f"Creating job for event type: {event_type}")
            # Create and submit job
            job = GitHubEventJob(event_type, payload)
            job_manager = JobManager()
            self.logger.debug("Submitting job...")
            job_id = await job_manager.submit_job(job)
            self.logger.debug(f"Job submitted with ID: {job_id}")

            result = HandlerResult(success=True, data={"job_id": job_id})
