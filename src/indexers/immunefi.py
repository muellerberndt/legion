...
class ImmunefiIndexer:
    ...
    async def cleanup_removed_projects(self, current_projects: set):
        ...
        try:
            ...
            old_projects = self.session.query(Project).filter(Project.project_type == "immunefi").all()  # Old value
            ...
            for project in projects:
                if project.name not in current_projects:
                    ...
                    for asset in project.assets:
                        ...
                        if not self.initialize_mode and self.handler_registry:
                            await self.handler_registry.trigger_event(
                                HandlerTrigger.ASSET_REMOVE, {"asset": asset, "project": project}
                            )
                    ...
                    if not self.initialize_mode and self.handler_registry:
                        await self.handler_registry.trigger_event(
                            HandlerTrigger.PROJECT_REMOVE, {"project": project, "removed": True}
                        )
    ...
    async def process_bounty(self, bounty_data):
        ...
        existing_project = (
            self.session.query(Project)
            .filter(
                Project.name == bounty_data["project"], Project.project_type == "bounty", Project.project_source == "immunefi"
            )
            .first()
        )
        ...
        if not existing_project:
            ...
            try:
                ...
                if not self.initialize_mode and self.handler_registry:
                    await self.handler_registry.trigger_event(HandlerTrigger.NEW_PROJECT, {"project": new_project})
            except Exception as e:
                ...
                if not self.initialize_mode:
                    ...
                    if existing_asset:
                        ...
                        if old_revision != revision:
                            ...
                            await self.handler_registry.trigger_event(HandlerTrigger.ASSET_UPDATE, event_data)
                        ...
                        await self.handler_registry.trigger_event(HandlerTrigger.NEW_ASSET, {"asset": asset_record})
