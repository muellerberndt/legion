...
def mock_notification_service():
    service = Mock(spec=MockNotificationService)
    with patch('src.services.notification_service.NotificationService.get_instance', return_value=service):
        yield service
...
def sample_project():
    ...
    project.extra_data = {"maxBounty": "100000"}
    project.assets = []
    return project
...
async def test_new_project_event(handler, sample_project, mock_notification_service):
    ...
    assert "maxBounty: 100000" in message
...
async def test_no_changes_update(handler, sample_project, mock_notification_service):
    ...
    handler.context = {
        'project': sample_project,
        'old_project': sample_project  # Same project, no changes
    }
