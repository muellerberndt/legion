...
async def test_handle_webhook_valid_payload(watcher, test_client):
    ...
    test_payload = {
        "payload": [
            {  # Wrap in payload array to match Quicknode format
    ...
    resp = await client.post("/webhooks/quicknode", json=test_payload, headers={"Content-Type": "application/json"})
...
async def test_handle_webhook_invalid_payload(watcher, test_client):
    ...
    # Send invalid JSON
    resp = await client.post("/webhooks/quicknode", data="invalid json", headers={"Content-Type": "application/json"})
...
async def test_event_triggering_without_event_bus(watcher, test_client):
    ...
    test_payload = {"eventId": "test-event-123", "blockNumber": "0x123", "transactionHash": "0xabc..."}
    ...
    try:
        resp = await new_client.post("/webhooks/quicknode", json=test_payload, headers={"Content-Type": "application/json"})
